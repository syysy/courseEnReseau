	...
	case StateRun:
		...
		if finished {
			...
			if <-g.channel_lecture == 5 {
				...
				t0, err := strconv.Atoi(cleanTime(ttemp[0]))
				if err != nil {
					return err
				}
				t1, err := strconv.Atoi(cleanTime(ttemp[1]))
				if err != nil {
					return err
				}
				t2, err := strconv.Atoi(cleanTime(ttemp[2]))
				if err != nil {
					return err
				}
				t3, err := strconv.Atoi(cleanTime(ttemp[3]))
				if err != nil {
					return err
				}
...
func cleanTime(strtemp string) string {
